...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (isFormValid) {
      setIsLoading(true);
      try {
        // Dynamically routing to your admin or user signin endpoints
        ...
        // Handle post-login logic here (e.g., save tokens, redirect to dashboard)
        // window.location.href = formData.isAdmin ? "/admin/dashboard" : "/dashboard";
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-2xl shadow-md border border-stone-100">
      <h2 className="text-xl font-bold text-slate-800 mb-5">Sign In to Your Account</h2>
      
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        
        {globalError && (
          <div className="p-3 text-xs font-medium text-red-600 bg-red-50 rounded-xl flex items-center gap-2 border border-red-200">
            <AlertCircle size={16} />
        ...
        <div className="space-y-1">
          ...
          {touched.email && errors.email && (
            <p className="text-[11px] text-red-500 font-medium flex items-center gap-1 pl-0.5">
        ...
        {/* SIGN IN AS ADMIN SWITCH BUTTON */}
        <div className="pt-1">
          <label className="relative flex items-center gap-3 cursor-pointer select-none group w-fit">
            ...
            <div className="w-5 h-5 border-2 border-stone-300 rounded-md bg-white transition-all duration-200 flex items-center justify-center group-hover:border-rose-400 peer-checked:bg-rose-500 peer-checked:border-rose-500 shadow-xs">
              <div className="w-2 h-2 bg-white rounded-xs opacity-0 scale-0 transition-all duration-200 peer-checked:opacity-100 peer-checked:scale-100" />
            </div>
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full mt-2 bg-rose-500 hover:bg-rose-600 text-white font-bold text-sm py-3.5 rounded-xl transition-colors shadow-md hover:shadow-lg active:scale-[0.99] transform select-none disabled:opacity-50 disabled:cursor-not-allowed"
        >
